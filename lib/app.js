	...
	function ponerAreas() {
        ...
        listaAtracciones.setAttribute("class", "listaAtracciones");
				...
				for (var j = 0; j < atracciones.length; j++) {
					...
					try{
					var atraccion = atracciones[j].getElementsByTagName("altura_minima")[0];
					var nombreAtraccion = atraccion.firstChild.nodeValue;
					}catch{
					...
					try{
					var atraccion = atracciones[j].getElementsByTagName("altura_maxima")[0];
					var nombreAtraccion = atraccion.firstChild.nodeValue;
					}catch{
					...
					//Tiempos espera
					const datosJSON = tiemposJSON();
					
				
						console.log(datosJSON[2])
			...
			async function tiemposJSON() {
				return fetch("lib/tiempos_espera.json")
				.then((res) =>{
		...
		Object.entries(shows).forEach(elem => {
			...
			showsArea.forEach(elem => {
				...
				if (elem.hours.length != 0) {
					...
					mensajeError.innerHTML = "No hay horarios disponibles para este show";
