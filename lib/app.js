...
function areasXML(){
  //Obtener datos de areas por el XML
  ...
  //Consolear error si no va el XML 
  catch(error) {
    console.log("Error");
    return null;
  }
...
async function llamarAPI() {
  fetch("https://samuelencinas.dev/shows_parque/P01")
    ...
    //Mostrar mensaje error si falla la API
...
function mostrarEspectaculos(espectaculos){
  ...
  Object.entries(shows).forEach(elem =>{
    ...
    showsArea.forEach(elem =>{
      ...
      //Si no hay horas para el espectaculo
      else{
  ...
  //Mostrar Error si los datos son truchos
  catch (error){
    mensajeError();
